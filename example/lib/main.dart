...
class _MyAppState extends State<MyApp> {
  ...
  _list() async {
    List returned;
    try {
      returned = await EscPosUsbPlugin.getUSBDeviceList;
  ...
  _connect(int vendor, int product) async {
    bool returned;
    try {
      returned = await EscPosUsbPlugin.connectPrinter(vendor, product);
    } on PlatformException {
      //response = 'Failed to get platform version.';
    }
    if(returned){
  ...
  _print() async {
    try {
      await EscPosUsbPlugin.printText("Testing ESC POS printer...");
    } on PlatformException {
      //response = 'Failed to get platform version.';
    }
  }
